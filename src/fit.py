...
#@profile
def minimize(fn, x0, MaxIter = 300, fgrad = None, callback = None, DEBUG = True, **kwargs):
    nx = x0.shape[0]
    ...
    if multi:
        if DEBUG:
            print "Fitting: using cores to evaluate objective function"
        p = ProcessingPool()
    else:
        if DEBUG:
            print "Fitting: serial specified or failed to load multiprocessing module, using single core"
    ...
    for iter in range(MaxIter):
        ...
        y = y_new
        if DEBUG:
            print "%4d %20.12f %20.12f %20.12f"%(iter, y, la.norm(g), la.norm(dx))
            sys.stdout.flush() 
...
if __name__ == "__main__":
    #log.verbose = "DEBUG1"
    x, y, converge_pattern = minimize(lambda x: x[0]**2 + x[1]**4 + 2*x[1]**2 + 2*x[0] + 2., np.asarray([10., 20.]), MaxIter = 500)
    print x
    print y
    print converge_pattern
    #log.result("x = %s\ny=%20.12f", x, y)
